...
function table(parentid,id,metadata, hasBtn=true){
    ...
    thead.style.background=('#461220')
    var tbody = createElement('tbody');
    tbody.style.color=('#461220')
...
function fillTable(tableid,obs,modifyFunction=null,deletefunction=null,printfunction=null){
    ...
    for(var i=0; i<obs.length; i++){
        ...
        if(hasButton){
            ...
            buttondel.classList.add('btn-sm');
...
function textFieldBinder(field,pattern,obj,prop,oldobj){
    ...
    if(val!==""){
        ...
        ob[prop] = null; //array ekak widihata property pass karanwa
    }
...
function radioBinder(field,pattern,obj,prop,oldobj,bindvalue){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
...
function fileUpload(field,pattern,obj,prop,oldobj,imgViewPhoto){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
    ...
    reader.onload = function (){
        ob[prop] = btoa(reader.result);
        imgViewPhoto.style.display = 'block';
        imgViewPhoto.src = reader.result;
    }
...
function comboBox2Binder(field,pattern,obj,prop,oldobj,parentid){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
...
function fileChooser(parent,id,lblText,labelWidth,extenstions,maxsize,required,ob,prop,oldob){
    ...
    label.innerHTML = lblText + " (100*100)";
    label.for = id;
    label.classList.add('control-label');
    label.classList.add('font-weight-bold');
   // label.classList.add('col-md-4'+labelWidth);
    ...
    fieldButtonC.style.border = "1px solid #d6d6c2";
    ...
    fieldButtonC.style.width = "150px";
    fieldButtonC.style.height = "40px";
        ...
        if(size>maxsize){
            ...
            if(match){
                ...
                reader.onload = function(e) {
                  ...
                  //  ob[namproperty] = filename;
                    ul.appendChild(li);
                    var li = createElement('li');
                    li.innerHTML = bytesToSize(size);
                    //  ob[sizeproperty] = bytesToSize(size);
                    ul.appendChild(li);
